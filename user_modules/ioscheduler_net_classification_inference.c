...
static const int ioscheduler_test_list[33] = {
...
static int current_ioscheduler_val = 256;

void data_processing(ioscheduler_class_net *ioscheduler) {
  int sample_count;
  ...
  for (test_idx = 0; test_idx < 33; test_idx++) {
    int ret =
        snprintf(processing_file_name, 255,
                 "../ml-models-analyses/ioscheduler-per-disk/feature_inputs/"
                 "feature_input_readrandomwriterandom_%d_data.csv",
                 ioscheduler_test_list[test_idx]);
    ...
    for (traverse_idx = 0; traverse_idx < sample_count; ++traverse_idx) {
      ...
      if (ioscheduler_data_processing(data, (ioscheduler_net *)ioscheduler,
                                    true, false)) {
        class =
            predict_ioscheduler_class(ioscheduler);
        printf("predicted class %d\n", class);
      }
    }
    ioscheduler_data_processing(data, (ioscheduler_net *)ioscheduler,
                              true, true);
...
void simulation(ioscheduler_class_net *ioscheduler) {
  ...
  int ret = snprintf(
      test_file_name, 255,
      "/home/abbas/python/testinput.csv");
  if (!(ret > 0 && ret < 255)) {
    kml_assert(false);
  }
  ...
  sample_matrix = allocate_matrix(sample_count, n_features, ioscheduler->type);
  ...
  foreach_mat(sample_matrix, rows, row_idx) {
    ...
    matrix *indv_result = ioscheduler_class_net_inference(feed, ioscheduler);
...
int main(void) {
  ...
  ioscheduler->state.is_training = false;
  set_weights_biases_from_file(
      ...
      "../ml-models-analyses/ioscheduler/user_nn_arch_data/linear0_bias.csv");
  set_weights_biases_from_file(
      ioscheduler->layer_list->layer_list_head->next->next,
      "../ml-models-analyses/ioscheduler/user_nn_arch_data/linear1_w.csv",
      "../ml-models-analyses/ioscheduler/user_nn_arch_data/linear1_bias.csv");
  set_weights_biases_from_file(
      ioscheduler->layer_list->layer_list_tail,
      "../ml-models-analyses/ioscheduler/user_nn_arch_data/linear2_w.csv",
      "../ml-models-analyses/ioscheduler/user_nn_arch_data/linear2_bias.csv");

  //data_processing(ioscheduler);

  for (int i = 0; i < num_run; i++) {
    simulation(ioscheduler);
    reset_ioscheduler_class_net(ioscheduler);
    iter = 0;
  }
